...
% Most of the configurations are the same as that of the 1D scenario in 
% 2-FD_GPR_sim.m


% load dielectric permittivity (epsilon)
ep = struct2array(load('Synthetic/Data/2D/yTrue2D_ep.mat'))';


row_height = size(ep,2); %depth of dz=0.05
width = size(ep,1); %number of shots in x direction

%define epsilon (dielectric permt.), mu (magnetic permeab.),
% sigma ( conductivity)
...
save('Synthetic/Data/2D/fdraw_2D.mat','co_data');
